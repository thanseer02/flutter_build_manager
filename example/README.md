...
This is a production-quality example application for the `flutter_release_manager` package. It serves as both documentation and an integration test bed.

## Project Structure

- **`lib/main.dart`**: Contains a minimal UI demonstrating the application identity.
- **`pubspec.yaml`**: Configured to depend locally on `flutter_release_manager` using `path: ../`.
...
### 3. Initialize
```bash
fvm dart run flutter_release_manager:flutter_release init
```
...
### 7. Build AAB
```bash
fvm dart run flutter_release_manager:flutter_release build --target aab
```
*Expected output: Same as APK, but generates an Android App Bundle.*

### 8. Build IPA
```bash
fvm dart run flutter_release_manager:flutter_release build --target ipa
```
...
- **Pubspec errors**: Ensure `flutter_release_manager` is referenced correctly in dependencies using `path: ../`.
